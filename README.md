...
最终得到的不是一句「可能因为业绩指引不及预期」，而是一份可以自己复查证据的研究页面。
...
股票代码为必填项。异动日期、基准指数、行业 ETF、同行公司、输出语言和具体调查问题都可以自定义。未指定输出语言时，报告默认使用简体中文（`zh-CN`）。
...
> 打开报告时需要联网加载图表库，以及访问报告内保留的原始来源链接。
...
这个项目提供的是研究流程，不是交易信号，也不构成投资建议。价格异动附近出现一条新闻，并不代表两者已经存在因果关系；请阅读原始来源并自行判断。
